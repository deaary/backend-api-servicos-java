...
public class ChamadoController {
    ...
    // aqui vamos definir o endpoint para o servico de cadastro de um novo chamado
    ...
    public ResponseEntity<Chamado> cadastrarChamado(@PathVariable Integer idCliente ,@RequestBody Chamado chamado){
        chamado = chamadoService.cadastrarChamado(chamado, idCliente); // nesse momento o chamado ja foi cadastrado no database
                // agora precisamos criar o caminho (URI) para que esse novo chamado possa ser acessado
    ...
    // vamos fazer o mapeamento do metodo de atribuir um funcionario a um determinado chamado
    ...
    public ResponseEntity<Chamado> modificarStatus(@PathVariable Integer idChamado,
        ...
        Chamado chamado = chamadoService.modificarStatus(idChamado, status);
        return ResponseEntity.ok().body(chamado);
    }
}
